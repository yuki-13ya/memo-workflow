...
| `discord-ingest/catch_up_discord.py` | キューの最終取得位置からDiscord投稿を遡及取得し、新規投稿バッチを作るCLI | 未取得投稿の回収範囲を変えたとき |
...
| `run_memodump_sync_ui.cmd` / `run_memodump_sync_ui.vbs` / `show_memodump_sync_ui.ps1` | コンソールを表示せず登録済みMemodumpタスクを任意のタイミングで起動し、完了結果とinboxを確認するWindows UI | 手動同期UIや表示内容を変えたとき |
